...
class Demo_Importer_Plus_AJAX {
	...
	private $api_url = DEMO_IMPORTER_PLUS_MAIN_DEMO_URI . 'wp-json/demoimporterplusapi/v1/dipa-demos';
	/**
	 * Class Consructor
	 */
	...
	public function required_plugin( $required_plugins = array(), $options = array(), $enabled_extensions = array() ) {
		...
		if ( isset( $_POST['required_plugins'] ) ) {
			$sanitized_required_plugins = array_map(
				...
				$_POST['required_plugins']
			);
		}

		$required_plugins = ( isset( $_POST['required_plugins'] ) ) ? $sanitized_required_plugins : $required_plugins;
		...
		$options            = ( isset( $_POST['options'] ) ) ? demo_importer_plus_clean_vars( json_decode( $_POST['options'] ) ) : $options;
		$enabled_extensions = ( isset( $_POST['enabledExtensions'] ) ) ? demo_importer_plus_clean_vars( json_decode( $_POST['enabledExtensions'] ) ) : $enabled_extensions;
	...
	public function required_plugin_activate( $init = '', $options = array(), $enabled_extensions = array() ) {
		...
		$options            = ( isset( $_POST['options'] ) ) ? demo_importer_plus_clean_vars( json_decode( stripslashes( $_POST['options'] ) ) ) : $options;
		$enabled_extensions = ( isset( $_POST['enabledExtensions'] ) ) ? demo_importer_plus_clean_vars( json_decode( stripslashes( $_POST['enabledExtensions'] ) ) ) : $enabled_extensions;
