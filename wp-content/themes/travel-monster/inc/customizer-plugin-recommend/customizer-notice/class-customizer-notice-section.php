...
/**
 * Travel_Monster_Customizer_Notice_Section class
 */
class Travel_Monster_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'travel_monster_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'travel_monster_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $travel_monster_recommended_plugins as $slug => $plugin_opt ) {
			...
			$travel_monster_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$travel_monster_recommended_plugin['class'] = 'active';
			} else {
				$travel_monster_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$travel_monster_recommended_plugin['button_class'] = 'install-now button';
					$travel_monster_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$travel_monster_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$travel_monster_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$travel_monster_recommended_plugin['button_class'] = 'deactivate-now button';
					$travel_monster_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$travel_monster_recommended_plugin['id'] = $slug;
			$travel_monster_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$travel_monster_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$travel_monster_recommended_plugin['description'] = $info->short_description;
			}

			$travel_monster_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $travel_monster_recommended_plugin;
